...
const PracticeList = () => {
    const [practices, setPractices] = useState([]); // Lưu danh sách bài tự luyện
    const searchParams = useSearchParams();
    const router = useRouter();
    const user = useCurrentUser();
    const { isLoading } = useAuth();

    // Hàm lấy danh sách bài tự luyện
    const refreshList = async () => {
        try {
            const response = await API.get("/practice/get-all");
            setPractices(response.data);
        } catch (error) {
            console.error("Lỗi khi lấy danh sách bài tự luyện:", error);
        }
    };
            ...
            <div className="flex min-h-screen">
                ...
                <div className="px-10 py-6 w-[85%] border-l-2 border-gray-300/50 shadow-sm">
                    <div className="flex justify-between items-center mb-6">
                        <h2 className="text-2xl font-bold">Danh sách vòng thi</h2>
                    </div>
