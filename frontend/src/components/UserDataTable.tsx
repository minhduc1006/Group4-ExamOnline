...
export function DataTable<TData, TValue>({
  ...
  const handleLock = async (id: number, isLocked: boolean) => {
    ...
    const path = isLocked ? `/unlock/${id}` : `/lock/${id}`;
    try {
      // eslint-disable-next-line @typescript-eslint/no-unused-vars
      const res = await API.put(
        `${process.env.NEXT_PUBLIC_API_URL}/user` + path,
        {
          headers: { "Content-Type": "application/json" },
        }
      );
  ...
  const openLockOpen = (rowData: TData) => {
    ...
    <>
              ...
              table.getRowModel().rows.map((row, index) => (
                ...
                >
                  ...
                  <TableCell className="text-left flex justify-start gap-2">
                    ...
                    <button
                      type="button"
                      onClick={() => openLockOpen(row.original)}
                      className="p-2 bg-orange-500 text-white rounded-lg hover:bg-orange-600 hover:scale-105 transition-all duration-300 ease-in-out"
                    >
                      {(row.original as { isLocked: boolean }).isLocked
                        ? "Mở khóa"
                        : "Khóa"}
                    </button>
                  </TableCell>
      ...
      <Dialog open={lockOpen} onOpenChange={setLockOpen}>
        ...
        >
          <DialogHeader>
            <DialogTitle>
              {(selectedRow as boolean)?.isLocked ? "Mở khóa tài khoản" : "Khóa tài khoản"}{" "}
              {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
              {selectedRow ? (selectedRow as any).username : ""}
            </DialogTitle>
          </DialogHeader>
          <DialogFooter>
            <button
              ...
              onClick={() => {
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                handleLock(
                  (selectedRow as number)?.id,
                  (selectedRow as boolean)?.isLocked
                );
              }}
            >
              {(selectedRow as boolean)?.isLocked ? "Mở khóa" : "Khóa"} tài khoản
            </button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
