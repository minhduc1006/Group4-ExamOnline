...
const AddManagerForm = ({ role, onSuccess }: { role: string; onSuccess: () => void }) => {
  ...
  const handleAddManager = async (
    data: z.infer<typeof managerAccountSchema>
  ) => {
    setFormError("");
    try {
      ...
      setFormError("Tên tài khoản đã tồn tại");
    }
  };
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent aria-describedby={undefined} className="sm:max-w-[600px] z-[999] bg-white">
        <Form {...form}>
          <form onSubmit={form.handleSubmit(handleAddManager)}>
            <DialogHeader>
