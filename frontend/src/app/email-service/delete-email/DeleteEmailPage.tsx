...
import { DELETE_EMAIL } from "@/helper/urlPath";
...
const AddEmail = () => {
  ...
  useEffect(() => {
    const request = async () => {
      if (!token) {
        setMessage("Xóa email thất bại");
      } else {
        try {
          const { data: mailDeleted } = await axios.put(
            `${process.env.NEXT_PUBLIC_API_URL}${DELETE_EMAIL}`,
            {
              token: token
            },
            {
          ...
          setMessage("Xóa email thành công");
